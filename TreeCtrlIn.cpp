...
int CTreeCtrlIn::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Create base node
	this->InsertItem(_T("Input files"), CTreeCtrlKeyword::workImageIn, CTreeCtrlKeyword::workImageIn);
...
{
	CTreeCtrlNode selectedNode = GetSelectedItem();
	if (selectedNode != NULL && selectedNode != this->GetRootItem())
	{
...
void CTreeCtrlIn::OnFileCloseAll() 
{
	...
	CTreeCtrlNode fileNode = this->GetRootItem().GetChild();
	ASSERT(fileNode);
